...
每日可选的简历策略和多简历映射已经配置化：`daily-options` 记录当天选择，`preferences.json` 保存用户自己的附件文件名与岗位方向映射。当前公开版尚不自动发送附件，这是一个待完善项。
...
专用浏览器启动脚本主要在 **Windows** 上开发和人工验证。CI 现在会在 Linux 和 Windows 上都跑一遍测试套件，但浏览器启动这一步本身目前只在 Windows 上做过人工验证。macOS/Linux 上的 Agent 可以读取 Skill，但需要使用等价的可见 Chrome/Edge 启动命令，目前不宣称已验证。
...
测试覆盖跨 Agent 安装、重复安装、更新备份、私有工作区初始化、审核与批准门禁、开场白保存、台账校验和隐私扫描。`npm test` 完全离线运行，不需要网络。CI 通过 GitHub Actions 在 `windows-latest` 和 `ubuntu-latest` 上分别用 Node 22 跑一遍。
